...
1. **`n_channels` / `phase` 必填**：基类 `assert "n_channels" in kwargs`（构造函数现为 `**kwargs`），不再有 `n_channels_default=3` 静默默认；`phase` 同样必填并校验为 `train`/`test`/`val`。`self.opt` 已去除，下游改读 `self.phase` / `self.n_channels`。
...
3. **子类超参「无默认 + `_demand` 失败 loud」**（2026-07-25 二次收紧 + 三次收紧 sigma_test + 四次收紧改 `**kwargs`）：初版我们保留 `opt.get('X', D)` 并写理由注释，但那仍属「静默默认」——既违反本项目 §3「不设默认值除非有明确理由」，更关键的冲突是：sigma/patch_size/num_patches 正是 `GridSearch` 要扫的超参，把它们写死成类内默认值，等于把「该由上层注入的超参」藏进了类内部，形成埋雷（实例化漏传就静默拿到 64/25/40/3000，与 sweep 意图悄悄脱节）。现改为：**所有超参（含 `sigma_test`）全部必填**，经基类实例方法 `BaseDataset._demand(self._rest, key)` 取值（缺失即抛出**自动取自 `type(self).__name__` 的 owner-tagged** `AssertionError`，绝不静默回退；调用处只写 `self._demand(self._rest, 'H_size')`，不用传类名）。原本在 `DnCNN`/`DnPatch` 留了一个「派生型默认」`sigma_test = opt.get('sigma_test', self.sigma)`（测试等级==训练 sigma），但那只是「test==train」假设下的巧合，若 sweep 在异于训练的等级测试却漏传 `sigma_test` 会静默回退——同样是雷，故**三次收紧**时一并改为 `_demand` 必填，四类数据集 `sigma_test` 现已统一无默认。**四次收紧**：构造函数从 `def __init__(self, opt)` 改为 `def __init__(self, **kwargs)`，把项目 §4 的 kwargs 纪律落到数据层——基类消费 `n_channels`/`phase`/`paths_H`/`paths_L` 后把剩余 key 存入 `self._rest` 交给子类；子类用 `self._demand(self._rest, 'X')` 取值（key 被 `pop`，不会误判为残留），消费完末尾 `assert not self._rest`，**任何拼写错 / schema 漂移的未知 key 都 fail loud**。`tests/data/` 现有 6 个 `test_*_init_missing_required_raises`（覆盖漏传超参）+ 新增 `test_dncnn_init_unknown_key_raises`（覆盖未知 key），锁死「漏传 / 错传超参即 fail loud」。
...
**kwargs 纪律已对齐**：数据层构造函数统一为 `**kwargs`，且**强制无残留未知 key**——基类消费自身 key 后把剩余存入 `self._rest`，子类 `_demand` 逐个 `pop` 消费，末尾 `assert not self._rest`（拼错 key 即 fail loud）。这把项目 §4 的「该存在的必存在 + 不该存在的不能存在」落到数据层，与自动化层一致；原先「opt 不穷举未知 key」的留白已**关闭**。

...
3. **子类超参「无默认 + `_demand` 失败 loud」**（2026-07-25 二次收紧 + 三次收紧 sigma_test）：初版我们保留 `opt.get('X', D)` 并写理由注释，但那仍属「静默默认」——既违反本项目 §3「不设默认值除非有明确理由」，更关键的冲突是：sigma/patch_size/num_patches 正是 `GridSearch` 要扫的超参，把它们写死成类内默认值，等于把「该由上层注入的超参」藏进了类内部，形成埋雷（实例化漏传就静默拿到 64/25/40/3000，与 sweep 意图悄悄脱节）。现改为：**所有超参（含 `sigma_test`）全部必填**，经基类实例方法 `BaseDataset._demand(opt, key)` 取值（缺失即抛出**自动取自 `type(self).__name__` 的 owner-tagged** `AssertionError`，绝不静默回退；调用处只写 `self._demand(opt, 'H_size')`，不用传类名）。原本在 `DnCNN`/`DnPatch` 留了一个「派生型默认」`sigma_test = opt.get('sigma_test', self.sigma)`（测试等级==训练 sigma），但那只是「test==train」假设下的巧合，若 sweep 在异于训练的等级测试却漏传 `sigma_test` 会静默回退——同样是雷，故**三次收紧**时一并改为 `_demand` 必填，四类数据集 `sigma_test` 现已统一无默认。`tests/data/` 现有 6 个 `test_*_init_missing_required_raises`，其中 DnCNN/DnPatch 的用例已覆盖 `sigma_test` 缺失即 fail loud，锁死「漏传超参即 fail loud」，防止日后重构偷偷加回默认。
